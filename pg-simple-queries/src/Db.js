...
const FATAL_MESSAGES = /ENOTFOUND|ECONNREFUSED|ECONNRESET|ETIMEOUT|Connection terminated unexpectedly/
...
class Db extends EventEmitter {
  ...
  constructor ({
    ...
    // NOT setting the schemaVersion or schemaBase is DEPRECATED
    // keeping the code for a while longer though
    if (typeof schemaVersion !== 'number' || !schemaBase) {
      const callsites = getCallsites()
      let cwd = __dirname
      if (callsites.length > 0) {
        cwd = callsites[1].getFileName()
      }
      const appPackage = readPkgUp.sync({cwd}).pkg
      if (typeof schemaVersion !== 'number' && PROD) {
        schemaVersion = parseFloat(appPackage.version)
      }
      if (!schemaBase) {
        schemaBase = appPackage.name
      }
    }
  ...
  __poolUp () {
    ...
    this.pool.on('error', (err) => {
      if (err.message.match(FATAL_MESSAGES)) {
        this.__fatal(new Error(`the connection pool had connection error: ${err}. Exiting.`))
        process.exit(1)
      } else {
  ...
  async __schemaListen () {
    ...
    client.on('error', async (err) => {
      ...
      if (err.message.match(FATAL_MESSAGES)) {
        this.__fatal(new Error(`connection error on client listening for schema notifications: ${err}. Exiting.`))
        process.exit(1)
      }
      await this.__schemaListen()
      await client.end()

...
class MigrationHelper {
  ...
  async __setUpNewDb () {
    ...
    try {
      ...
      await this.__postVersion(migrationInfoForSchemaVersion.postMigrations)
  ...
  async __createMigrationsInfo (sqlFiles: AnnotatedSqlFile[]): Promise<MigrationsInfo[]> {
    ...
    return versionSet.map(version => {
      const includeIdempotents = version === versionForIdempotents
      const relevantFiles = sqlFiles.filter(file => {
        if (file.type === 'idempotent') return includeIdempotents && !fileHasSnapAtVersion(file, version, sqlFiles)
        else return file.version === version
      })
      return {
        version,
